...
class BrandVoiceValidator {
  ...
  private async testContentAnalysis(): Promise<void> {
    await this.runTest('Content Analysis', async () => {
      const testContent = "Our innovative AI-powered solution helps optimize your marketing strategy efficiently. We provide cutting-edge automation tools for modern businesses.";
      ...
      if (typeof result.voiceScore !== 'number' || result.voiceScore < 0 || result.voiceScore > 100) {
  ...
  private async testVoiceScoring(): Promise<void> {
    await this.runTest('Voice Scoring', async () => {
      const testCases = [
        {
          content: "Our professional solution optimizes business efficiency through innovative strategies.",
      ...
      for (const testCase of testCases) {
        ...
        if (typeof score !== 'number' || score < testCase.expectedRange[0] || score > testCase.expectedRange[1]) {
          throw new Error(`Score ${score} not in expected range ${testCase.expectedRange} for: ${testCase.content}`);
        }
      }
    });
  }
  ...
  private async testSuggestionGeneration(): Promise<void> {
    await this.runTest('Suggestion Generation', async () => {
      ...
      for (const suggestion of result.suggestions) {
        if (!suggestion.type || !suggestion.issue || !suggestion.suggestion || !suggestion.priority) {
  ...
  private async testGuidelinesRetrieval(): Promise<void> {
    await this.runTest('Guidelines Retrieval', async () => {
      ...
      if (!guidelines.tone || !guidelines.vocabulary || !guidelines.style || !guidelines.messaging) {
  ...
  private async testPerformance(): Promise<void> {
    await this.runTest('Performance Test', async () => {
      const longContent = "Professional business solution that optimizes marketing efficiency through innovative AI-powered automation. ".repeat(50);
