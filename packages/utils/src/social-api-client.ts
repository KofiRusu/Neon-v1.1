...
export class SocialApiClient {
  ...
  async getTrendingHashtags(platform: string): Promise<string[]> {
    const baseTags = [
      ...
      'love', 'life', 'style', 'daily', 'motivation'
    ];
  ...
  private generateMockTrends(platform?: string): TrendData[] {
    const platforms = platform ? [platform] : ['instagram', 'tiktok', 'twitter', 'facebook', 'linkedin'];
    const keywords = [
      'artificial intelligence', 'sustainable fashion', 'remote work',
      'healthy lifestyle', 'digital marketing', 'cryptocurrency',
      'mental health', 'travel', 'food trends', 'technology'
    ];
