...
async function main() {
  ...
  if (branchName.startsWith('ui/') || branchName.startsWith('agent-ui/') || branchName.startsWith('design-system/')) {
    checks.push(
      { name: 'Accessibility Check', command: 'npm run test:a11y || echo "⚠️ A11y check not available"', always: false },
      { name: 'Visual Regression', command: 'npm run test:visual || echo "⚠️ Visual tests not available"', always: false }
    );
  }
  ...
  if (['main', 'staging'].includes(branchName)) {
    checks.push(
      { name: 'E2E Tests', command: 'npm run test:e2e', always: false }
    );
  }
  ...
  if (passed) {
    ...
    console.log(`🚀 Ready to deploy to ${branchName === 'main' ? 'production' : branchName === 'staging' ? 'staging' : 'development'}`);
    process.exit(0);
  } else {
