...
import { EmailMarketingAgent, type EmailSequenceInput, type PersonalizationInput, type EmailPerformanceData, type ABTestInput } from "@neon/core-agents";
...
export const emailRouter = createTRPCRouter({
  ...
  generateSequence: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        logger.info('Generating email sequence', { topic: input.topic, audience: input.audience }, 'EmailRouter');
        
        const agent = new EmailMarketingAgent();
        const result = await agent.generateSequence(input as EmailSequenceInput);
        
        logger.info('Email sequence generated successfully', { sequenceId: result.sequenceId }, 'EmailRouter');
        ...
        logger.error('Failed to generate email sequence', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
  ...
  personalizeEmail: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        logger.info('Personalizing email content', { userTraits: Object.keys(input.userTraits) }, 'EmailRouter');
        
        const agent = new EmailMarketingAgent();
        const result = await agent.personalize(input as PersonalizationInput);
        
        logger.info('Email personalized successfully', { score: result.personalizationScore }, 'EmailRouter');
        ...
        logger.error('Failed to personalize email', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
        throw new Error(`Failed to personalize email: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }),
  ...
  analyzePerformance: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        logger.info('Performance analysis completed', { score: result.score, campaignId: input.campaignId }, 'EmailRouter');
        ...
        logger.error('Failed to analyze performance', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
  ...
  runABTest: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        logger.info('Creating A/B test', { name: input.name, variants: input.variants.length }, 'EmailRouter');
        
        const agent = new EmailMarketingAgent();
        const result = await agent.runABTest(input as ABTestInput);
        
        logger.info('A/B test created successfully', { testId: result.testId, winner: result.winner }, 'EmailRouter');
        ...
        logger.error('Failed to create A/B test', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
  ...
  sendCampaign: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        logger.info('Sending email campaign', { name: input.name, recipientCount: input.recipients.emails.length }, 'EmailRouter');
        ...
        logger.info('Email campaign sent successfully', { campaignId: result.data?.campaignId }, 'EmailRouter');
        ...
        logger.error('Failed to send campaign', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
  ...
  getTemplates: publicProcedure
    .query(async () => {
      try {
        ...
        logger.error('Failed to get templates', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
  ...
  generateSubjectLines: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        logger.info('Generating subject lines', { topic: input.topic, count: input.count }, 'EmailRouter');
        ...
        logger.info('Subject lines generated successfully', { count: result.data?.subjectLines?.length }, 'EmailRouter');
        ...
        logger.error('Failed to generate subject lines', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
        throw new Error(`Failed to generate subject lines: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }),
  ...
  optimizeSendTimes: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        logger.info('Optimizing send times', { audienceSize: input.audience.length }, 'EmailRouter');
        ...
        logger.info('Send times optimized successfully', { optimalTimes: result.data?.optimalTimes?.length }, 'EmailRouter');
        ...
        logger.error('Failed to optimize send times', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
        throw new Error(`Failed to optimize send times: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }),
  ...
  segmentAudience: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        logger.info('Audience segmented successfully', { segmentCount: result.data?.segments?.length }, 'EmailRouter');
        ...
        logger.error('Failed to segment audience', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
        throw new Error(`Failed to segment audience: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }),
  ...
  createNewsletter: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        logger.info('Creating newsletter', { title: input.title, topicCount: input.topics.length }, 'EmailRouter');
        ...
        logger.info('Newsletter created successfully', { newsletterId: result.data?.newsletter?.id }, 'EmailRouter');
        ...
        logger.error('Failed to create newsletter', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
        throw new Error(`Failed to create newsletter: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }),
  ...
  getAnalytics: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        logger.info('Getting email analytics', { timeRange: input.timeRange, campaignCount: input.campaignIds?.length }, 'EmailRouter');
        ...
        const mockAnalytics = {
          ...
          topPerforming: [
            { id: 'camp_1', name: 'Welcome Series #3', openRate: 45.2, clickRate: 18.6, sent: 1250 },
            { id: 'camp_2', name: 'Product Update January', openRate: 38.1, clickRate: 12.3, sent: 5420 },
        ...
        logger.info('Email analytics retrieved successfully', { 
        ...
        logger.error('Failed to get analytics', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
  ...
  getAgentStatus: publicProcedure
    .query(async () => {
      try {
        ...
        logger.error('Failed to get agent status', { error: error instanceof Error ? error.message : 'Unknown error' }, 'EmailRouter');
