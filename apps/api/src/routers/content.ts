...
export const contentRouter = router({
      ...
      const posts = [
        {
          ...
          hashtags: input.includeHashtags ? ['#neonhub', '#neonsigns', '#customdesign', `#${input.topic.replace(/\s+/g, '').toLowerCase()}`] : [],
        ...
        },
        {
          ...
          hashtags: input.includeHashtags ? ['#businessgrowth', '#neonlights', '#branding', '#signage'] : [],
  ...
  getContentAnalytics: publicProcedure
    .input(z.object({
      ...
      platform: z.enum(['instagram', 'facebook', 'tiktok', 'twitter', 'linkedin', 'all']).default('all'),
    }))
    .query(async ({ input }) => {
      // Mock analytics data
