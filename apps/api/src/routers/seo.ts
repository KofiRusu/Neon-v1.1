      ...
      const analysis = {
        ...
        suggestions: [
          ...
          },
          {
            type: 'content_length',
            severity: 'low',
            suggestion: input.content.length < 300 ? 'Consider expanding content to at least 300 words' : 'Content length is optimal',
            impact: 'Better content depth improves rankings'
          },
          {
            type: 'meta_optimization',
            severity: input.metaTitle ? 'low' : 'high',
            suggestion: input.metaTitle ? 'Meta title looks good' : 'Add a compelling meta title (50-60 characters)',
...
Contact us today for a free consultation and quote. Let's discuss how our ${input.targetKeywords[0]} solutions can transform your business.`,
  ...
  getPerformanceMetrics: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      // Calculate changes
      metrics.organicTraffic.change = ((metrics.organicTraffic.current - metrics.organicTraffic.previous) / metrics.organicTraffic.previous * 100);
      metrics.averagePosition.change = parseFloat(metrics.averagePosition.previous) - parseFloat(metrics.averagePosition.current);

      return {
        metrics,
        keywordPerformance: input.keywords?.map(keyword => ({
          keyword,
