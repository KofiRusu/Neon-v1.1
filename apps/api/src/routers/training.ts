...
const TrainingLogSchema = z.object({
  ...
  eventType: z.enum(['FINE_TUNING', 'RETRY', 'OPTIMIZATION', 'PERFORMANCE_UPDATE', 'MODEL_SWITCH', 'VALIDATION']),
...
const ChartDataQuery = z.object({
  ...
  eventType: z.enum(['FINE_TUNING', 'RETRY', 'OPTIMIZATION', 'PERFORMANCE_UPDATE', 'MODEL_SWITCH', 'VALIDATION']).optional(),
...
export const trainingRouter = createTRPCRouter({
  ...
  logTrainingEvent: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const scoreDelta = (input.scoreAfter && input.scoreBefore) 
  ...
  getPerformanceChart: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const stats = {
        ...
        eventTypeDistribution: logs.reduce((acc, log) => {
  ...
  getAgentTrends: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const agentTrends = logs.reduce((acc, log) => {
  ...
  getTrainingStats: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const stats = {
        ...
        avgScoreImprovement: logs.reduce((sum, log) => sum + (log.scoreDelta || 0), 0) / logs.length || 0,
        totalRetries: logs.reduce((sum, log) => sum + log.retryCount, 0),
        ...
        agentTypeBreakdown: logs.reduce((acc, log) => {
        ...
        performanceDistribution: {
          ...
          good: logs.filter(log => (log.scoreAfter || 0) >= 75 && (log.scoreAfter || 0) < 90).length,
          average: logs.filter(log => (log.scoreAfter || 0) >= 60 && (log.scoreAfter || 0) < 75).length,
          poor: logs.filter(log => (log.scoreAfter || 0) < 60).length,
        },
        
        improvementTrend: logs.filter(log => (log.scoreDelta || 0) > 0).length / logs.length * 100 || 0,
        
        topPerformingAgents: uniqueAgents
          .map(agent => {
            const agentLogs = logs.filter(log => log.agentId === agent.agentId);
            const avgScore = agentLogs.reduce((sum, log) => sum + (log.scoreAfter || 0), 0) / agentLogs.length || 0;
