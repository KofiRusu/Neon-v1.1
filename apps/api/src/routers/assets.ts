...
export const assetsRouter = createTRPCRouter({
  // Create a new asset
  createAsset: protectedProcedure.input(AssetSchema).mutation(async ({ ctx, input }) => {
    try {
  ...
  getAssets: publicProcedure.input(AssetQuerySchema).query(async ({ ctx, input }) => {
    const where: any = {};
  ...
  getAsset: publicProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
  ...
  // Update asset
  updateAsset: protectedProcedure.input(AssetUpdateSchema).mutation(async ({ ctx, input }) => {
    try {
  ...
  reviewAsset: protectedProcedure.input(AssetApprovalSchema).mutation(async ({ ctx, input }) => {
    try {
  ...
  getAssetStats: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const stats = {
        ...
        statusDistribution: assets.reduce(
          (acc, asset) => {
        ...
        typeDistribution: assets.reduce(
          (acc, asset) => {
        ...
        avgRating:
          assets.filter(a => a.rating).reduce((sum, asset) => sum + (asset.rating || 0), 0) /
  ...
  searchAssets: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const assets = await ctx.db.asset.findMany({
        where,
        take: input.limit,
        orderBy: [{ usage: 'desc' }, { rating: 'desc' }, { createdAt: 'desc' }],
        select: {
          id: true,
          name: true,
          type: true,
