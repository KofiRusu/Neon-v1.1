            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-secondary">{test.variants} variants • {test.metric}</p>
                </div>
              </div>
              <div className="flex items-center space-x-2">
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                  test.status === 'running' ? 'bg-neon-green text-black' :
                  test.status === 'completed' ? 'bg-neon-blue text-white' :
                  'bg-neon-orange text-black'
                }`}>
                  {test.status}
                </span>
                <button className="p-2 glass rounded-lg text-secondary hover:text-neon-blue">
                  {test.status === 'running' ? <StopIcon className="h-4 w-4" /> : <PlayIcon className="h-4 w-4" />}
                </button>
              </div>
            </div>
