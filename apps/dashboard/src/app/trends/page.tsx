...
export default function TrendsPage(): JSX.Element {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'viral': return 'text-neon-pink';
      case 'hot': return 'text-neon-orange';
      case 'trending': return 'text-neon-blue';
      case 'emerging': return 'text-neon-green';
  ...
  const getPredictionIcon = (prediction: string) => {
    switch (prediction) {
      ...
      case 'sustained': return PlayIcon;
      case 'early': return ClockIcon;
    ...
    <div className="min-h-screen p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
        {trendingTopics.map((trend) => {
          ...
          return (
            <div key={trend.id} className="card-neon group hover:scale-105 transition-all duration-300">
              <div className="flex items-center justify-between mb-4">
                <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${getStatusColor(trend.status)} bg-current bg-opacity-10`}>
                  {trend.status}
                </div>
                <div className="flex items-center space-x-2">
      ...
      <div className="glass-strong p-6 rounded-2xl">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-secondary">Enter a keyword to discover trending patterns and predictions</p>
        </div>
      </div>
    </div>
  );
