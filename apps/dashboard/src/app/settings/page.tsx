...
export default function SettingsPage(): JSX.Element {
  ...
  // Fetch settings
  const { data: settingsData, isLoading: settingsLoading, refetch: refetchSettings } = api.settings.getSettings.useQuery({
  ...
  // Fetch API keys
  const { data: apiKeysData, isLoading: apiKeysLoading, refetch: refetchAPIKeys } = api.settings.getAPIKeys.useQuery({
    includeInactive: true,
  });

  // Fetch prompt templates
  const { data: promptsData, isLoading: promptsLoading, refetch: refetchPrompts } = api.settings.getPromptTemplates.useQuery({
    category: selectedCategory || undefined,
          ...
          <div className="glass-strong p-6 rounded-2xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  {showSecretKeys ? <EyeSlashIcon className="h-4 w-4" /> : <EyeIcon className="h-4 w-4" />}
                </button>
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center space-x-3">
                              ...
                              <span className={`px-2 py-1 rounded-full text-xs ${getSettingTypeColor(setting.type)} bg-gray-800`}>
                                {setting.type}
                              </span>
                              {setting.isSystem && (
                            ...
                            <div className="mt-2">
                              ...
                              <div className="font-mono text-xs text-primary bg-gray-800 p-2 rounded">
                                {formatSettingValue(setting.value, setting.type, setting.isEncrypted && !showSecretKeys)}
                              </div>
                            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-secondary text-sm">Securely store and manage API keys for external services</p>
            </div>
          ...
          <div className="glass-strong p-6 rounded-2xl">
                  ...
                  return (
                    <div key={apiKey.id} className="card-neon">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className={`w-3 h-3 rounded-full ${apiKey.isActive ? 'bg-neon-green' : 'bg-gray-500'}`}></div>
                      </div>
            ...
            {(!apiKeysData?.apiKeys || apiKeysData.apiKeys.length === 0) && !apiKeysLoading && (
              <div className="text-center py-12">
                ...
                <p className="text-secondary mb-6">Add your first API key to enable external service integrations.</p>
                <button 
                  onClick={() => setShowAddAPIKey(true)}
                  className="btn-neon"
                >
          ...
          <div className="glass-strong p-6 rounded-2xl">
                ...
                {promptsData?.templates?.map((template: PromptTemplate) => (
                  <div key={template.id} className="card-neon">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="flex items-center space-x-2">
                        <div className={`w-2 h-2 rounded-full ${template.isActive ? 'bg-neon-green' : 'bg-gray-500'}`}></div>
                      </div>
                    </div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {template.variables.map((variable) => (
                            <span key={variable} className="px-2 py-1 bg-neon-purple/20 text-neon-purple text-xs rounded-full">
                              {variable}
                            </span>
                          ))}
                        </div>
            ...
            {(!promptsData?.templates || promptsData.templates.length === 0) && !promptsLoading && (
              <div className="text-center py-12">
                ...
                <p className="text-secondary mb-6">Create your first prompt template to standardize AI interactions.</p>
                <button 
                  onClick={() => setShowAddPrompt(true)}
                  className="btn-neon"
                >
